function startsystem() {
    ...
    function startepkex() {
        ...
        function yaypekxs() {
            ...
            if ((kepkxed == true) && (cmd_disable == false)) {
                kepkxed = false;
                $('.lycker').addClass('ly_st');
            } else if (kepkxed == false) {
                kepkxed = true;
                $('.lycker').removeClass('ly_st');
            }
    ...
    function convertponyst(github, thishere, data) {
        ...
        if (count_git_items == count_git_items_c) {
            ...
            // Get Pony List
            var listponylist = '';
            for (i = 0; i < ponylist_git.length; i++) {
                if (ponylist_git[i].ponylist != "") {
            ...
            for (i = 0; i < ponyinteractions_git.length; i++) {
                if (ponyinteractions_git[i].interactions != "") {
    ...
    function startponyconvert(data, totalfoder_git) {
        ...
        // Count Folders
        for (i = 0; i < totalfoder_git.length; i++) {
            if (totalfoder_git[i].type == "dir") {
    ...
    // Download Start
    $('.items_title').append($('<p>').text(document.title + ' made by Jasmin Dreasond (c)\n'));
    ...
    function get_ponies_git(data) {
        if ((data.user != null) && (data.user != undefined) && (data.user != '')) {

            // ERROR SYSTEM
            function errorresult(parterror, notcmd) {
                $('.items').append($("<command>").append($('<span>')
                ...
                scrolldown();
                if (notcmd != true) { cmd_disable = false; }
            }
            ...
            function startdownload_git(totalfoder_git, folderfinal) {
                ...
                // Start Convert
                startponyconvert({
                    'folder': data.user + '/' + data.repository + '/master/' + folderfinal,
                    'user': data.user,
                    'type': data.type,
            ...
            function folderlist_git(data_map) {
                $('.items').append($("<command>").append($('<div>', { class: 'help_title' }).text('Folder list of repository:')));
                for (i = 0; i < data_map.length; i++) {
                    if (data_map[i].type == "dir") {
            ...
            function startgithub() {
                ...
                // Start
                git_blog.fetch(function(err, res) {
                    if (err) { errorresult(1); } else {
                        git_blog.fetchBranches(function(err, res) {
                            if (err) { errorresult(2); } else {
                                ...
                                if (data.type == 'default') { master_name_git = 'master'; } else { master_name_git = prompt('Branch name:', master_name_git); }
                                var git_master = git_blog.getBranchByName(master_name_git);


                                if ((master_name_git == null) || (master_name_git == undefined)) { cmd_disable = false; } else if ((master_name_git == '') || (git_master == undefined)) {
                                    errorresult('branch name null or incorrect', true);
                                    setTimeout(function() { startgithub(); }, 100);
                                } else {

                                    // Enter Master
                                    git_master.fetchContents(function(err, res) {
                                        if (err) { errorresult(3); } else {
                                            ...
                                            function enter_subfolder_git(data3, repeatdetect) {
                                                if (repeatdetect == true) { git_dir2 = git_dir2.getDirByName(data3); } else { git_dir2 = git_dir.getDirByName(data3); }

                                                if (git_dir2 == undefined) {
                                                    errorresult('folder name null or incorrect', true);
                                                    setTimeout(function() { startgithub(); }, 100);
                                                } else {
                                                    git_dir2.fetchContents(function(err, res) {
                                                        if (err) { errorresult(5); } else {

                                                            // Start Download
                                                            if (data.type == 'default') { startdownload_git(git_dir2.getContents(), 'Content/Ponies/'); }
                                                            // Start Download
                                                            else {
                                                                var cmd_command_git = prompt('Directory\nFolder: " ./' + master_name_git + '/' + folder_git_select + ' "\n\nReset Command: reset.folder\nFolder List Command: folder.list', 'start.download');
                                                                if (cmd_command_git == 'start.download') { startdownload_git(git_dir2.getContents(), folder_git_select); } else if ((cmd_command_git == null) || (cmd_command_git == undefined) || (cmd_command_git == '')) { cmd_disable = false; } else if (cmd_command_git == 'reset.folder') { startgithub(); } else if (cmd_command_git == 'folder.list') {
                                                                    folderlist_git(git_dir2.getContents());
                                                                    setTimeout(function() { enter_subfolder_git(data3, repeatdetect); }, 100);
                                                                } else {
                                                                    folder_git_select = folder_git_select + cmd_command_git + '/';
                                                                    enter_subfolder_git(cmd_command_git, true);
                                                                }
                                                            }
                                            ...
                                            function enter_folder_git(data2) {
                                                git_dir = git_master.getDirByName(data2);

                                                if (git_dir == undefined) {
                                                    errorresult('folder name null or incorrect', true);
                                                    setTimeout(function() { startgithub(); }, 100);
                                                } else {
                                                    git_dir.fetchContents(function(err, res) {
                                                        if (err) { errorresult(4); } else {
                                                            ...
                                                            // Start Download
                                                                ...
                                                                var cmd_command_git = prompt('Directory\nFolder: " ./' + master_name_git + '/' + folder_git_select + ' "\n\nReset Command: reset.folder\nFolder List Command: folder.list', 'start.download');
                                                                if (cmd_command_git == 'start.download') { startdownload_git(git_dir.getContents(), folder_git_select); } else if ((cmd_command_git == null) || (cmd_command_git == undefined) || (cmd_command_git == '')) { cmd_disable = false; } else if (cmd_command_git == 'reset.folder') { startgithub(); } else if (cmd_command_git == 'folder.list') {
                                                                    folderlist_git(git_dir.getContents());
                                                                    setTimeout(function() { enter_folder_git(data2); }, 100);
                                                                } else {
                                                                    folder_git_select = folder_git_select + cmd_command_git + '/';
                                                                    enter_subfolder_git(cmd_command_git);
                                                                }
                                                            }
                                            ...
                                            function start_folder_git() {
                                                ...
                                                // Start Download
                                                    ...
                                                    var cmd_command_git = prompt('First Directory\n\nReset Command: reset.folder\nFolder List Command: folder.list', 'start.download');
                                                    if (cmd_command_git == 'start.download') { startdownload_git(git_master.getContents(), folder_git_select); } else if ((cmd_command_git == null) || (cmd_command_git == undefined) || (cmd_command_git == '')) { cmd_disable = false; } else if (cmd_command_git == 'reset.folder') { startgithub(); } else if (cmd_command_git == 'folder.list') {
                                                        folderlist_git(git_master.getContents());
                                                        setTimeout(function() { start_folder_git(); }, 100);
                                                    } else {
                                                        folder_git_select = folder_git_select + cmd_command_git + '/';
                                                        enter_folder_git(cmd_command_git);
                                                    }
                                                }
                                            }
                                            setTimeout(function() { start_folder_git(); }, 100);
    ...
    // CMD User
    function cmdsystem(starttype, autocommand) {
        if (cmd_disable == false) {
            function cmdsystemstart() {

                if (starttype == 'clear_st') { var start_cmd_type = ''; } else if (starttype == 'get') { var start_cmd_type = autocommand; } else { var start_cmd_type = 'help'; }
                ...
                // Get Source
                else if (cmd_result == "get_source") {
                    $('.items').append($("<command>").append($('<div>', { class: 'help_title' })
                ...
                // Extract
                    ...
                    scrolldown();
                    cmdsystem('get', 'extract ');
                }

                // Pony Code
                else if (cmd_result == "extract ponycode") {
                    chrome.storage.local.get({ baseponytext: '' }, function(settings) {
                        var blob = new Blob([settings.baseponytext], { type: "text/plain;charset=utf-8" });
                        saveAs(blob, "pc_m_ponycode.txt");
                    });
                    $('.items').append($("<command>").append($('<span>').text('ponycode - '), $('<span>', { class: "download_complete" }).text("File extracted successfully!")));
                    scrolldown();
                }

                // Interactions
                else if (cmd_result == "extract interactions") {
                    chrome.storage.local.get({ interactionstext: '' }, function(settings) {
                        var blob = new Blob([settings.interactionstext], { type: "text/plain;charset=utf-8" });
                        saveAs(blob, "pc_m_interactions.txt");
                    });
                    $('.items').append($("<command>").append($('<span>').text('interactions - '), $('<span>', { class: "download_complete" }).text("File extracted successfully!")));
                    scrolldown();
                }

                // Pony List
                else if (cmd_result == "extract ponylist") {
                    chrome.storage.local.get({ ponylisttext: '' }, function(settings) {
                        var blob = new Blob([settings.ponylisttext], { type: "text/plain;charset=utf-8" });
                        saveAs(blob, "pc_m_ponylist.txt");
                    });
                    $('.items').append($("<command>").append($('<span>').text('ponylist - '), $('<span>', { class: "download_complete" }).text("File extracted successfully!")));
                    scrolldown();
                }

                // Custom CSS
                else if (cmd_result == "extract custom_css") {
                    chrome.storage.local.get({ customcsstext: '' }, function(settings) {
                        var blob = new Blob([settings.customcsstext], { type: "text/plain;charset=utf-8" });
                        saveAs(blob, "pc_m_custom_css.css");
                    });
                    $('.items').append($("<command>").append($('<span>').text('custom css - '), $('<span>', { class: "download_complete" }).text("File extracted successfully!")));
                    scrolldown();
                }

                // Custom Language
                else if (cmd_result == "extract custom_language") {
                    chrome.storage.local.get({ customlanguagetext: '' }, function(settings) {
                        var blob = new Blob([settings.customlanguagetext], { type: "text/plain;charset=utf-8" });
                        saveAs(blob, "pc_m_constom_language.txt");
                    });
                    $('.items').append($("<command>").append($('<span>').text('custom language - '), $('<span>', { class: "download_complete" }).text("File extracted successfully!")));
                    scrolldown();
                }
                ...
                // help
                else if (cmd_result == "help") {
                    $('.items').append($("<command>").append($('<div>', { class: 'help_title' }).text("Command List:"),
